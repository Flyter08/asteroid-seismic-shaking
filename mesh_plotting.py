...
def plot_grid_2d(X, Y, z, title, filename=None, labels=("$x$", "$y$"), limits=(-2, 2), plot_rectangle=False, vertices=None,
    ...
    if plot_rectangle:
        rect = patches.Rectangle((0, 0), dim[0], dim[1], linewidth=1, edgecolor='r', facecolor='none', label="Bounding box")
    ...
    fig.colorbar(surf, aspect=5, orientation='vertical', shrink=0.5, label="[m/s²]")
...
def plot_polygon(ax, vertices, coordinate):
    ...
    if ACTIVATE:
        ...
        polygon = patches.Polygon(xy, closed=True, linewidth=1, edgecolor='r', facecolor='none', alpha=0.2)
